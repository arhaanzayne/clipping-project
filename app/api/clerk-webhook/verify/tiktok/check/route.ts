...
export async function POST(req: NextRequest) {
  ...
  const apifyRes = await fetch(
    `https://api.apify.com/v2/actor-tasks/YOUR_TIKTOK_APIFY_TASK_ID/run-sync?token=${process.env.APIFY_TOKEN}`,
    {
      method: "POST",
      body: JSON.stringify({
        usernames: [username],
  ...
  // Extract TikTok bio
  const bio = apifyData?.items?.[0]?.signature || "";

  // 3️⃣ Code match?
  if (!bio.includes(verification_code)) {
    return NextResponse.json({
      verified: false,
      message: "Code not found in TikTok bio",
    });
  }
  ...
  return NextResponse.json({
    verified: true,
    message: "TikTok account verified!",
  });
}
